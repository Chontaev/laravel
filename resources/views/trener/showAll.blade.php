@extends('admin')
    ...
    <div class="content pt-3 col-12">
      ...
      <div class="d-flex d-flex justify-content-between pt-4 pb-4 align-baseline">
        <a class="btn btn-dark btn-block btn-primary col-6" href="{{route('createTrener')}}"><h6>Создать нового тренера</h6></a>
      </div>
      <table class="table">
        <thead>
          <tr>
        ...
        <tbody>
          @foreach($data as $el)
            <tr>
              <td>{{$el->name}}</td>
              <td><img style="height: 50px;" src="{{ asset($el->img) }}" alt="break"></td>
